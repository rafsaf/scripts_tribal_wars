function processCollectTroopsScriptEn() {
  ...
  if (
    ...
    UI.ErrorMessage("Go to the Tribe -> Members -> Troops or Defence", "2e3");
  } else {
    const cacheTime = DATA_COLLECT_TROOPS.cacheTime * 60 * 1000;
    let output = "";
    ...
    if (params.get("mode") === "members_troops") {
      scriptName = "Army Collection";
      firstLine = DATA_COLLECT_TROOPS.firstLineTroops;
      showFirstLine = DATA_COLLECT_TROOPS.showFirstLineTroops;
      showNicknames = DATA_COLLECT_TROOPS.showNicknamesTroops;
      mode = "troops";
    } else {
      scriptName = "Deff Collection";
      firstLine = DATA_COLLECT_TROOPS.firstLineDeff;
      showFirstLine = DATA_COLLECT_TROOPS.showFirstLineDeff;
      showNicknames = DATA_COLLECT_TROOPS.showNicknamesDeff;
      mode = "defence";
    }
    ...
    async function RenderPlayerTroops() {
      const removedPlayers = DATA_COLLECT_TROOPS.removedPlayers.split(";");
      const today = new Date().getTime();
      const afterCacheTime = today + cacheTime;
      const storageDate = localStorage.getItem("troops-storageDate" + mode);
      let parseTime;

      if (today < storageDate && DATA_COLLECT_TROOPS.cache) {
        ...
        const newProgressPar = (i, len) => {
          return `<h1 style="margin-top:10px;font-size:40px">${i}/${len}</h1><h1>Wait...</h1>`;
        };
      ...
      Dialog.show(
        ...
        `<h2 style="width:600px;">${scriptName}:</h2>
              ${
                DATA_COLLECT_TROOPS.removedPlayers === ""
                  ? ""
                  : `<p>Ommited: ${DATA_COLLECT_TROOPS.removedPlayers}</p>`
              }${
          lackAccessPlayers === ""
            ? ``
            : `<h4>Attention, partial or complete lack of overview:</h4>` +
              lackAccessPlayers
        }<textarea rows="15" style="width:95%;margin-top:15px;margin-bottom:25px;">${
          showFirstLine ? firstLine + "\r\n" : ""
        }${output}</textarea>
                  <p style="text-align:right"><small>Generated ${parseTime}.</small></p>`
      );
    }
